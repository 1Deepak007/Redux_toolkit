...
import { markDone, removeTodo, updateTodo } from '../features/todo/todoSlice';
...
const Todos = ({enterEditMode}) => {

    const dispatch = useDispatch();
    const todos = useSelector(state => state.todos.todos);    // useSelector to get values (todos) from store (like name )

    return (
        <div style={{ display: 'flex', justifyContent: 'center', margin: '20px 0' }}>
            <table
                style={{ width: '80%', maxWidth: '800px', borderCollapse: 'collapse', border: '1px solid #ddd', borderRadius: '8px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)', overflow: 'hidden', backgroundColor: '#ffffff',  }} >
                <thead>
                    ...
                    {todos.map((todo, index) => (
                        ...
                        >
                            ...
                            <td style={{ padding: '12px', borderBottom: '1px solid #ddd', display: 'flex', alignItems: 'center', gap: '10px' }}>
                                <button onClick={() => dispatch(markDone(todo.id))} style={{ background: 'green', color: 'lightgreen', border: 'none', borderRadius: '4px', padding: '8px 12px', fontSize: '14px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background-color 0.3s ease', }} onMouseOver={(e) => (e.currentTarget.style.backgroundColor = '#00cc00')} onMouseOut={(e) => (e.currentTarget.style.backgroundColor = 'green')} >     <IoCheckbox style={{ marginRight: '5px' }} /> Done </button>
                                <button onClick={() => dispatch(enterEditMode(todo.id, todo.text))} style={{ background: 'green', color: 'lightgreen', border: 'none', borderRadius: '4px', padding: '8px 12px', fontSize: '14px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background-color 0.3s ease', }} onMouseOver={(e) => (e.currentTarget.style.backgroundColor = 'blue')} onMouseOut={(e) => (e.currentTarget.style.backgroundColor = 'blue')} >     <IoPencilSharp style={{ marginRight: '5px' }} /> Update </button>
